...
const DocumentUploader: React.FC<DocumentUploaderProps> = ({
  ...
  const handleRemoveFile = (id: string) => {
    setFiles(prev => {
      ...
      // If we're removing the last file, notify parent
  ...
  const handleGoogleDriveFile = async (file: File) => {
    ...
    if (!validTypes.includes(file.type)) {
      toast.error(`Invalid file type: ${file.name}. Only PDF, DOC, and DOCX files are supported.`);
      return;
    }
    ...
    // Analyze PDF for optimization warnings
    if (file.type === 'application/pdf') {
      try {
        const issues = await analyzePdfDocument(file);
        setOptimizationWarnings(prev => ({
          ...prev,
          [newFile.id]: issues
        }));
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setIsUploading(false);
        onUploadingChange?.(false);
        // Important: Pass the file to parent component
