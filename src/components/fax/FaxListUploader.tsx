...
import { FileSpreadsheet, X, Edit2, ArrowLeft, FileText } from 'lucide-react';
...
const FaxListUploader: React.FC<FaxListUploaderProps> = ({
  ...
  const handleGoogleDriveFileSelected = async (file: File) => {
    try {
      setIsUploading(true);
      ...
      toast.success(`Successfully loaded ${file.name} from Google Drive`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : `Failed to process file from Google Drive`);
    } finally {
      setIsUploading(false);
    }
  };
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          >
            ...
            <p className="text-xs text-gray-500 mt-1">
              Select spreadsheets from your Google Drive
            </p>
          </div>
        </div>
      </div>
